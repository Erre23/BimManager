...
namespace BimManager.Sunat.Entidad.Estructuras
{
    ...
    public class Invoice : IXmlSerializable
    {
        ...
        public void WriteXml(XmlWriter writer)
        {
            ...
            #region [UBLExtensions - Datos Adicionales Internos]

            //==========================
            //INICIO
            //==========================
            var ext3 = UblExtensions.Extension3.ExtensionContent;
            writer.WriteStartElement("ext:UBLExtension");
            #region ExtensionContent
            writer.WriteStartElement("ext:ExtensionContent");

            #region AdditionalInformation
            writer.WriteStartElement("DatosAdicionales");
            {


                #region AdditionalProperty
                {
                    foreach (var DatoAdicional in ext3.DatosAdicionales_Internos.DatoAdicional)
                    {
                        writer.WriteStartElement("DatoAdicional");

                        #region [Codigo]

                        writer.WriteElementString("Codigo", DatoAdicional.Codigo);

                        #endregion

                        #region Value

                        writer.WriteElementString("Valor", DatoAdicional.Valor);
            ...
            writer.WriteStartElement("sac:AdditionalInformation");
            {
...

                {
                    ...
                    {
                        //Validamos que el valor de los subtotal (gravada, exonerada, inafecta, gratuita) sea mayor a cero (0) para agregar la etiqueta sino la obviamos 
                        //A pesar que en el guia de elaboración de la boleta y factura todos los campos son Obligatorios o Mandatarios a raíz de unas reglas de validación
                        //implementadas 08/10/2017 en el servidor de SUNAT solo se debe agregar etiquetas de los códigos que tengan valor mayor a cero (0).
                        if (additionalMonetaryTotal.PayableAmount.value > 0)
                        {
...

                            {
                                writer.WriteStartElement("cbc:PayableAmount");
                                {
                                    ...
                                    writer.WriteValue(additionalMonetaryTotal.PayableAmount.value.ToString(Constantes.Formatos.FormatoNumerico, Formato));
                            ...
                            if (additionalMonetaryTotal.Percent > 0)
                            {
                                writer.WriteElementString("cbc:Percent",
                                    additionalMonetaryTotal.Percent.ToString(Constantes.Formatos.FormatoNumerico));
                            }
                            #endregion
                ...
                if (!string.IsNullOrEmpty(ext2.SunatEmbededDespatchAdvice.DeliveryAddress.ID))
                {
                    writer.WriteStartElement("sac:SUNATEmbededDespatchAdvice");
                    {
                        ...
                        writer.WriteStartElement("cbc:GrossWeightMeasure");
                        {
                            ...
                            writer.WriteValue(ext2.SunatEmbededDespatchAdvice.GrossWeightMeasure.Value.ToString(Constantes.Formatos.FormatoNumerico, Formato));
                        }
                        writer.WriteEndElement();
                        #endregion
                    }
            ...
            if (PrepaidPayment != null)
            {
                writer.WriteStartElement("cac:PrepaidPayment");
                {
                    ...
                    writer.WriteStartElement("cbc:PaidAmount");
                    {
                        ...
                        writer.WriteValue(PrepaidPayment.PaidAmount.value.ToString(Constantes.Formatos.FormatoNumerico, Formato));
                    }
                    writer.WriteEndElement();
            ...
            writer.WriteStartElement("cac:LegalMonetaryTotal");
            {
                if (LegalMonetaryTotal.AllowanceTotalAmount.value > 0)
                {
                    writer.WriteStartElement("cbc:AllowanceTotalAmount");
                    {
                        ...
                        writer.WriteValue(LegalMonetaryTotal.AllowanceTotalAmount.value.ToString(Constantes.Formatos.FormatoNumerico, Formato));
                    }
                    writer.WriteEndElement();
                }
                if (LegalMonetaryTotal.PrepaidAmount.value > 0)
                {
                    writer.WriteStartElement("cbc:PrepaidAmount");
                    {
                        ...
                        writer.WriteValue(LegalMonetaryTotal.PrepaidAmount.value.ToString(Constantes.Formatos.FormatoNumerico, Formato));
                    }
                    writer.WriteEndElement();
                }
                writer.WriteStartElement("cbc:PayableAmount");
                {
                    ...
                    writer.WriteValue(LegalMonetaryTotal.PayableAmount.value.ToString(Constantes.Formatos.FormatoNumerico, Formato));
                }
                writer.WriteEndElement();
            ...
            foreach (var invoiceLine in InvoiceLines)
            {
                ...
                writer.WriteValue(invoiceLine.InvoicedQuantity.Value.ToString(Constantes.Formatos.FormatoNumerico, Formato));
                writer.WriteEndElement();
                #endregion
                ...
                writer.WriteValue(invoiceLine.LineExtensionAmount.value.ToString(Constantes.Formatos.FormatoNumerico, Formato));
                writer.WriteEndElement();
                #endregion
                ...
                foreach (var item in invoiceLine.PricingReference.AlternativeConditionPrices)
                {
                    ...
                    writer.WriteValue(item.PriceAmount.value.ToString(Constantes.Formatos.FormatoNumerico, Formato));
                    writer.WriteEndElement();
                    #endregion
                ...
                if (invoiceLine.AllowanceCharge.ChargeIndicator)
                {
                    ...
                    writer.WriteValue(invoiceLine.AllowanceCharge.Amount.value.ToString(Constantes.Formatos.FormatoNumerico, Formato));
                    writer.WriteEndElement();
                    #endregion
                ...
                #region TaxTotal
                {
                    foreach (var taxTotal in invoiceLine.TaxTotals)
                    {
                        ...
                        if (!string.IsNullOrEmpty(taxTotal.TaxableAmount.currencyID))
                        {
                            ...
                            writer.WriteString(taxTotal.TaxableAmount.value.ToString(Constantes.Formatos.FormatoNumerico, Formato));
                            writer.WriteEndElement();
                        }
                        ...
                        if (taxTotal.TaxSubtotal.Percent > 0)
                            writer.WriteElementString("cbc:Percent", taxTotal.TaxSubtotal.Percent.ToString(Constantes.Formatos.FormatoNumerico, Formato));
                ...
                writer.WriteString(invoiceLine.Price.PriceAmount.value.ToString(Constantes.Formatos.FormatoNumerico, Formato));
                writer.WriteEndElement();
