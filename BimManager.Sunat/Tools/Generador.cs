...
namespace BimManager.Sunat.Tools
{
    public static class Generador
    {
        public static Invoice GenerarInvoice(DocumentoElectronico documento)
        {
            ...
            // Datos Adicionales a la Factura segun Sunat
            ...
            //Datos Adicionales a la Factura Internos
            if (documento.DatoAdicionales_Internos != null && documento.DatoAdicionales_Internos.Count > 0)
            {
                invoice.UblExtensions.Extension3 = new UBLExtension();
                foreach (var adicional in documento.DatoAdicionales_Internos)
                {
                    invoice.UblExtensions.Extension3.ExtensionContent
            ...
            foreach (var DocumentoElectronicoDetalle in documento.Items)
            {
                var linea = new InvoiceLine
                {
                    ID = DocumentoElectronicoDetalle.Id,
                    InvoicedQuantity = new InvoicedQuantity
                    {
                        unitCode = DocumentoElectronicoDetalle.UnidadMedida,
                        Value = DocumentoElectronicoDetalle.Cantidad
                    },
                    LineExtensionAmount = new PayableAmount
                    {
                        currencyID = documento.Moneda,
                        value = DocumentoElectronicoDetalle.TotalVenta
                    },
                    ...
                    Item = new Item
                    {
                        Description = DocumentoElectronicoDetalle.Descripcion,
                        SellersItemIdentification = new SellersItemIdentification
                        {
                            ID = DocumentoElectronicoDetalle.CodigoItem
                        }
                    },
                    Price = new Price
                    {
                        ...
                        {
                            currencyID = documento.Moneda,
                            value = DocumentoElectronicoDetalle.PrecioUnitario
                        }
                    },
                };
                ...
                linea.TaxTotals.Add(new TaxTotal
                {
                    ...
                    {
                        currencyID = documento.Moneda,
                        value = DocumentoElectronicoDetalle.Impuesto
                    },
                    TaxSubtotal = new TaxSubtotal
                    {
                        TaxAmount = new PayableAmount
                        ...
                        TaxCategory = new TaxCategory
                        {
                            TaxExemptionReasonCode = DocumentoElectronicoDetalle.TipoAfectacionIgv,
                            TaxScheme = new TaxScheme()
                            {
                                ID = "1000",
                                Name = "IGV",
                ...
                if (DocumentoElectronicoDetalle.ImpuestoSelectivo > 0)
                    linea.TaxTotals.Add(new TaxTotal
                    {
                        TaxAmount = new PayableAmount
                        {
                            currencyID = documento.Moneda,
                            value = DocumentoElectronicoDetalle.ImpuestoSelectivo
                        },
                        TaxSubtotal = new TaxSubtotal
                        {
                            TaxAmount = new PayableAmount
                ...
                linea.PricingReference.AlternativeConditionPrices.Add(new AlternativeConditionPrice
                {
                    PriceAmount = new PayableAmount
                    {
                        ...
                        value = documento.Gratuitas > 0 ? 0 : DocumentoElectronicoDetalle.PrecioReferencial
                    },
                    PriceTypeCode = DocumentoElectronicoDetalle.TipoPrecio
                });
                ...
                if (documento.Gratuitas > 0)
                    linea.PricingReference.AlternativeConditionPrices.Add(new AlternativeConditionPrice
                    {
                        ...
                        {
                            currencyID = documento.Moneda,
                            value = DocumentoElectronicoDetalle.PrecioReferencial
                        },
                        PriceTypeCode = "02"
                    });
        ...
        public static CreditNote GenerarCreditNote(DocumentoElectronico documento)
        {
            ...
            // Datos Adicionales a la nota de credito segun SUNAT
            foreach (var adicional in documento.DatoAdicionales)
            {
            ...
            //Datos Adicionales a la boleta Internos
            if (documento.DatoAdicionales_Internos != null && documento.DatoAdicionales_Internos.Count > 0)
            {
                creditNote.UBLExtensions.Extension3 = new UBLExtension();
                foreach (var adicional in documento.DatoAdicionales_Internos)
                {
                    creditNote.UBLExtensions.Extension3.ExtensionContent
                        .DatosAdicionales_Internos.DatoAdicional.Add(new Entidad.Estructuras.DatoAdicional
                        {
                            Codigo = adicional.Codigo,
                            Valor = adicional.Valor
        ...
        public static DebitNote GenerarDebitNote(DocumentoElectronico documento)
        {
            ...
            // Datos Adicionales a la nota de debito según Sunat.
            foreach (var adicional in documento.DatoAdicionales)
            {
            ...
            //Datos Adicionales internos relacionados a la nota de credito
            foreach (var DatoAdicional in documento.DatoAdicionales_Internos)
            {
            ...
            foreach (var detalleDocumento in documento.Items)
            {
                ...
                linea.TaxTotals.Add(new TaxTotal
                {
                    ...
                    TaxSubtotal = new TaxSubtotal
                    {
                        ...
                        TaxCategory = new TaxCategory
                        {
                            TaxExemptionReasonCode = detalleDocumento.TipoAfectacionIgv,
                            TaxScheme = new TaxScheme()
                            {
                                ID = "1000",
                                Name = "IGV",
        ...
        public static SummaryDocuments GenerarSummaryDocuments_2018(ResumenDiario resumen)
        {
            var summary = new SummaryDocuments
            {
                ID = resumen.IdDocumento,
            ...
            foreach (var grupo in resumen.Resumenes2018)
            {
                var linea = new VoidedDocumentsLine
                {
                    LineID = grupo.Id,
                    DocumentTypeCode = grupo.TipoDocumento,
                    DocumentSerialID = grupo.Serie,
                    DocumentNumberID = grupo.Correlativo,

                    AccountingCustomerParty = new AccountingSupplierParty
                    {
                        AdditionalAccountID = grupo.DocumentoIdentidadTipo,
                        CustomerAssignedAccountID = grupo.DocumentoIdentidadNumero
                    },

                    BillingReference =
                        (grupo.Afecta_TipoDocumento != null && grupo.Afecta_TipoDocumento.Trim() != "" ?
                            new BillingReference { InvoiceDocumentReference = new InvoiceDocumentReference { DocumentTypeCode = grupo.Afecta_TipoDocumento, ID = grupo.Afecta_Serie + "-" + grupo.Afecta_Correlativo.ToString() } }
                            : null),

                    StatusConditionCode = grupo.EstadoItem,
                ...
                //Verificamos si existen otros tributos
